    ...
    while (1) {
        ...
        while (i < check) {
            if (read_buf[i] >= '0' && read_buf[i] <= '9') {
                digits_buf[indx1++] = read_buf[i];
            } else {
                nodigits_buf[indx2++] = read_buf[i];
            }
            ++i;
        }

        check_write_1 = write_bytes(out_yes, digits_buf, indx1);
        check_write_2 = write_bytes(out_no, nodigits_buf, indx2);
